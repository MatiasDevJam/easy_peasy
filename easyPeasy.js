...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/en-cartelera':
			res.write('----------------------------\n');
			res.write('	En Cartelera'.toUpperCase() + '\n');
			res.write('----------------------------\n');
			res.write('\n\n\n');
			res.write(`Total de películas en cartelera: ${movies.length} \n`);
			res.write('-----------------------------------\n')
		...
		case '/mas-votadas':
			...
			res.write('----------------------------\n');
			res.write('	Más votadas'.toUpperCase() + '\n');
			res.write('----------------------------\n');
			let votadas = movies.filter(function(elemento){
				return elemento.vote_average >= 7;
			});
			
			res.write(`Total de películas: ${votadas.length}`.toUpperCase() + '\n\n\n');
			res.write('----------------------------\n\n\n');
			...
			res.write(`Rating Promedio: `.toUpperCase() + `${promedio.toFixed(2)}` + '\n\n\n');
			res.write('----------------------------\n\n\n');
			   
			

			votadas.forEach(function(element){
				res.write(`TÍTULO: ${element.title}\nRATING: ${element.vote_average}\nDESCRIPCIÓN: ${element.overview}\n\n`)
			});
		...
		case '/sucursales':
			res.write('	--------------------------------\n');
			res.write('		Nuestras Sucursales'.toUpperCase() + '\n');
			res.write('	--------------------------------\n\n\n\n');
			
			res.write(`		Total de salas : `.toUpperCase() + `${theaters.length}` + '\n\n\n');


			theaters.forEach(function(movie){
				res.write('------------------------------------------------\n')
				res.write(`NOMBRE: ${movie.name} \nDIRECCIÓN: ${movie.address} \nDESCRIPCIÓN: ${movie.description}`);
				res.write('\n\n\n');
			});
		...
		case '/preguntas-frecuentes':
			...
			res.write(`Total de preguntas: ${faqs.length}`.toUpperCase() + '\n\n\n');
			
			faqs.forEach(function(movie){
				res.write('------------------------------------------------\n')
				res.write(`PREGUNTA: ${movie.faq_title} \nRESPUESTA: ${movie.faq_answer}`);
				res.write('\n\n\n');
			});
